...
```{r Strip all before 1996, cache = TRUE}
...
```{r Stripping down the EVTYPE values pt 1}
...
``` {r Stripping down the EVTYPE values pt 2 Electric Boogaloo}
...
```{r Final cleanup of the evtypes}
...
case.
```{r Reforming StormData, cache=TRUE}
...
```{r Calculating the spot metrics}
